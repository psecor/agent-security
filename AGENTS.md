...
status: in-progress
status_description: "Open-sourced and in production for project scans (Semgrep + Gitleaks + Trufflehog, Claude triage, Express + OAuth + bearer API on :3046, React UI at /security/, daily systemd timer); host-scanning is end-to-end across CLI, API + UI, category-aware Claude triage of critical/high CVEs (schema v2, Trivy runner, hosts/ findings), and a multi-host fan-out where every host scans itself and pushes into a shared findings repo (run-daily.sh pulls before scanning + pushes after; deploy/setup.md §8)."
...
Host-scanning is now end-to-end across CLI, API, UI, **and the daily-timer multi-host deploy story**. The findings JSON schema bumped to v2 with a `kind: "project" | "host"` discriminator and optional CVE fields on findings, host scan output types live in `service/src/scanner/host-types.ts` with corresponding writers in `apply.ts`, and a `HostToolRunner` interface (parallel to `ToolRunner`) is in `service/src/scanner/tools/host-types.ts` with a bundled Trivy runner at `service/src/scanner/tools/trivy.ts` (scoped to OS-package vulns, with noisy dirs skipped). The CLI grew a `--host` mode (and `SCAN_HOST=true` opt-in for `--all`) that runs `gatherHostInfo` (uname + /etc/os-release + dpkg-query) → host selector (package-set sha + 24h daily floor) → `scanHost` orchestrator → category-aware Claude triage of critical/high raws (mediums/lows stay on the naive path with linux-* → kernel-cve, else package-cve) → `findings/hosts/<name>.{json,md}` → bot commit. The server reads `findings/hosts/*.json`, exposes `/security/api/hosts*` endpoints, and the SPA has Hosts + Host pages. `run-daily.sh` now pulls the findings repo before scanning (best-effort, mirroring the existing best-effort push at the end), so a multi-host fleet — every host scans itself and pushes its own `findings/hosts/<host>.json`, the UI host pulls everyone's into its local clone — stays in sync without any central coordination; `deploy/setup.md` §8 walks through bringing up a scanner-only peer.
...
- Hybrid analysis: Semgrep (working-tree static analysis), Gitleaks (pattern-based secret detection across full git history), and Trufflehog (broader detector set with live API verification, also full git history) bundled as default tool runners; Claude does triage, prioritization, severity ranking, and rationale. The `ToolRunner` interface is small enough that adding `bandit`, `npm audit`, etc. is a TS file plus a one-line registration in `run.ts`.
...
11. ✅ Host scanning end-to-end: schema v2 with `kind` discriminator, host output types + writers, `HostToolRunner` interface, bundled Trivy runner, CLI/selector/orchestrator, hosts API + UI surface, category-aware Claude triage of critical/high CVEs, and per-host daily-timer multi-host deploy walkthrough (`deploy/setup.md` §8 + `run-daily.sh` pulling the findings repo before each scan).
12. ✅ Trufflehog bundled as third project runner (broader detector set than Gitleaks — LaunchDarkly without surrounding-context, Snowflake credential triplets, hundreds more — plus live API verification: verified findings → HIGH, unverified → MEDIUM so Claude triage can downrank the noisy bucket).
13. Future: dismissals, PR suggestions, Stop-hook trigger, AGENTS.md summary block.
...
│   └── host-scanning.md       scoping doc for host scans as a peer to project scans (CLI + API + UI + multi-host timer fan-out all wired)
...
│   └── hosts/                 per-host scan output (`<host>.{json,md}`); writers, CLI, API, UI, and multi-host daily-timer fan-out all wired
├── service/
│   ├── package.json
│   ├── tsconfig.json
│   ├── .env.example
...
│       │   │   ├── gitleaks.ts     bundled: full-history pattern-based secret detection
│       │   │   ├── trufflehog.ts   bundled: full-history secret detection with live API verification
...
14. **Multi-host fan-out is "every host scans itself, all push to one findings repo"** — there is no SSH-out and no agent-on-target. `deploy/setup.md` §8 walks through bringing up a peer: clone the findings repo on the peer, set `SCAN_HOST=true` + `HOST_NAME=<stable name>` + `FINDINGS_DIR=<peer's clone>` in its `service/.env`, drop the same scanner systemd timer in. `run-daily.sh` does a best-effort `git pull --rebase --autostash origin HEAD` from the findings repo before scanning and a best-effort `git push origin HEAD` after, so peers see each other's commits without any central coordination. The central UI host's findings clone picks up new peers' files on its next pull, no UI-side configuration required.
...
16. **Two peers with the same `HOST_NAME` will overwrite each other** — host findings land at `findings/hosts/<HOST_NAME>.{json,md}`, where `HOST_NAME` defaults to `os.hostname()`. If two boxes both default to `ubuntu` (cloud images, fresh installs), the second push to the shared findings repo will silently clobber the first's file — same path, same key. Always set `HOST_NAME` explicitly on every peer (`HOST_NAME=web-01`, `HOST_NAME=db-01`, …) before its first scan, and pick names you won't want to change later — renaming a host orphans the old `findings/hosts/<old>.json` file in the repo until someone deletes it by hand.
